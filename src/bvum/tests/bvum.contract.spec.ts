 ...
 *
 * Asserts: response SHAPE (value int kobo, ceiling kobo|null, recommendedPlan, windowDays:30),
 * that a near-ceiling business gets recommendedPlan set to the next plan up, that the plan is
 * NEVER mutated by the read, and owner-only auth (staff -> 403, no token -> 401).
 */
describe('BVUM (contract)', () => {
  ...
  const MARKET_CEILING = 200_000_000; // ₦2M in kobo (starter/market)
  ...
  it('GET /bvum as owner -> 200 + shape { value int kobo, ceiling kobo, recommendedPlan, windowDays:30 }', async () => {
    ...
    // market ceiling = ₦2M = 200,000,000 kobo
  ...
  it('GET /bvum for a near-ceiling business -> recommendedPlan set to the next plan up; plan NOT changed', async () => {
    ...
    // next plan up with a higher ceiling than market (₦2M) is business (₦20M)
